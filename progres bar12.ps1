...
for ($i = 0; $i -lt $duration; $i++) {
    ...
    # Progress bar - ПРАВИЛЬНЫЙ СПОСОБ!
    $bars = [math]::Floor($percent / 5)
    $progressBar = "#" * $bars
    $emptyBar = "." * (20 - $bars)
    
    # Цвет радуги в зависимости от процента
    if ($percent -lt 15) { $color = 'Red' }
    elseif ($percent -lt 30) { $color = 'DarkYellow' }
    elseif ($percent -lt 45) { $color = 'Yellow' }
    elseif ($percent -lt 60) { $color = 'Green' }
    elseif ($percent -lt 75) { $color = 'Cyan' }
    elseif ($percent -lt 90) { $color = 'Blue' }
    else { $color = 'Magenta' }
